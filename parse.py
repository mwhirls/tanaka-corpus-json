...
def load_sentences(tsv_path):
    with open(tsv_path, "r", encoding="utf-8") as tsvf:
        reader = csv.DictReader(
            tsvf,
            fieldnames=[SentenceField.ID, SentenceField.LANG, SentenceField.TEXT],
            delimiter="\t",
        )
        sentences = map(
            lambda row: Sentence(row[SentenceField.ID], row[SentenceField.TEXT]), reader
        )
...
def load_indices(tsv_path):
    with open(tsv_path, "r", encoding="utf-8") as tsvf:
        reader = csv.DictReader(
            tsvf,
            fieldnames=[
                IndexField.SENTENCE_ID,
                IndexField.TRANSLATION_ID,
                IndexField.TEXT,
            ],
            delimiter="\t",
        )
        indices = map(
            lambda row: Index(
                row[IndexField.SENTENCE_ID],
                row[IndexField.TRANSLATION_ID],
                row[IndexField.TEXT],
            ),
            reader,
        )
...
def to_json(jpn_sentences_dict, eng_sentences_dict, indices_dict):
    entries = [
        to_json_entry(v, eng_sentences_dict, indices_dict)
        for v in jpn_sentences_dict.values()
    ]
